...
have_synergy_group = numpy.random.choice([True, False], p = [1, 0])
synergy_allow = []
    ...
    if the_group.get('name') == 'Miracles':
        for card in mb_spinner.get():
            if "Creature" in card.get('type_line') and (card.get('name') not in ['Ice-Fang Coatl', 'Monastery Mentor', 'Snapcaster Mage', "Uro, Titan of Nature's Wrath", 'Baleful Strix']):
                ...
                print(f"Omitting {card.get('name')}")
...
for t in [(spells_target, mb_spell_spinner, ["Threat", "Answer"]), (mb_target_size, mb_land_spinner, ["Basic", "Fetch", "Dual", "Utility"])]:
    if t[1] == mb_spell_spinner:
        ...
        if Snow:
            for pair in [("Plains", "Snow-Covered Plains"),("Island", "Snow-Covered Island")\
            ,("Swamp", "Snow-Covered Swamp"),("Mountain", "Snow-Covered Mountain"),("Forest", "Snow-Covered Forest")]:
        ...
        for card in master_list:
            if 'Land' in card.get('type_line'):
                # print(f"name: {card.get('name')}\nmana production: {card.get('mana_production')}")
                # print(f"type line: {card.get('type_line')}")
                dual = 'Dual' in card.get('custom_keywords')
                fetch = (('Fetch' in card.get('custom_keywords')) & ('Prismatic Vista' not in \
                            card.get('name')))
    ...
    while len(the_deck.get('mb')) < t[0]:
        new_card = t[1].spin()
        if (mtg.card_in_list_number(new_card, the_deck.get('seventyfive')) < 4) or \
                ("Basic" in card.get('type_line')):
            ...
            for keyword in t[2]:
                if (mtg.keyword_in_list_number(keyword, "custom_keywords", current_mb) < \
                        keyword_minima[keyword]):
...
for term in keyword_minima:
    print(f"{term}")
    print(f"min: {keyword_minima[term]} total: {mtg.keyword_in_list_number(term, 'custom_keywords', the_deck.get('mb'))}")
